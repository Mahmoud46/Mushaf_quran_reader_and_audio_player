...
document.querySelector('.menu').addEventListener('click', e => {
    ...
    surahsMenu.classList.contains('active') ? e.target.innerText = 'menu_open' : e.target.innerText = 'menu';
});
...
function getSurahUsingNumber(surahNumber) {
    ...
    surahAyahs.innerHTML = '';
    surahasContainer.querySelector('p.active')?.classList.remove('active');
    surahasContainer.querySelectorAll('p')[surahNumber - 1].classList.add('active');
        ...
        if (surahNumber == surah.number) {
            ...
            surah.ayahs.forEach(ayah => {
                ...
                if (ayah.numberInSurah == 1) {
                    if (ayah.text.split(' ').slice(4).join(' ') == '' && ayah.text.split(' ').splice(0, 4).join(' ') == '﻿بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ') {
                        document.querySelector('.bsm.active')?.classList.remove('active');
                        insertAyahsCard(ayah, en_ayah);
                    } else if (ayah.text.split(' ').splice(0, 4).join(' ') != 'بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ') {
                        insertAyahsCard(ayah, en_ayah);
                        document.querySelector('.bsm.active')?.classList.remove('active');
                        console.log(true);
                    }
                        ...
                        surahAyahs.innerHTML += `
                            <div class="ayah-card">
                                <p>
                                    ...
                                    <i>&#1757 <small>${en_ayah.numberInSurah}</small></i>
                                </p>
                                <p> 
                                    ${en_ayah.text}
                                </p>
...
function getEnglishAyah(surahName, ayahNumber) {
    let en_ayah = {};
    en_surahs.forEach(surah => surah.name == surahName ? surah.ayahs.forEach(ayah => ayah.numberInSurah == ayahNumber ? en_ayah = ayah : null) : null);
    return en_ayah;
}
